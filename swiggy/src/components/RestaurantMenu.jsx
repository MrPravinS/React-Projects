...
const RestaurantMenu= () =>{
    ...
    const fetchMenu =  async() => {
     ...
     const itemCard = resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.carousel[0]?.dish?.info?.name;
 ...
 return (
    <div>
        ...
        <h2>Menu</h2>
        <ul>
            <Link><li>{resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1].card.card.carousel[0].dish.info.name} - {resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1].card.card.carousel[0].dish.info.price/100 || resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1].card.card.carousel[0].dish.info.defaultPrice}</li></Link>
